  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.section}>
        ...
        <FileUpload onFileUpload={handleEncode} label="Upload Image to Encode" />
        {originalImage && <img src={originalImage.src} alt="Original Image" className={styles.image} />}
        {encodedImage && <img src={encodedImage} alt="Encoded Image" className={styles.image} />}
      </div>

      {/* Decoding Section */}
      <div className={styles.section}>
        <h3>Decode Message</h3>
        <FileUpload onFileUpload={handleDecode} label="Upload Encoded Image to Decode" />
        <canvas ref={canvasRef} className={styles.canvas} />
        {decodedMessage && <div className={styles.decodedMessage}>Decoded Message: {decodedMessage}</div>}
      </div>
    </div>
  );
};
